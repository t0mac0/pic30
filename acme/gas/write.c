...
static
#endif
relax_addressT relax_align PARAMS ((relax_addressT addr, int align));
...
#ifdef TC_PIC30
      /*
      **   Apply +1 to the alignment power, since frags
      **   are always built in bytes, not address units.
      */
      if (alignment)
        alignment += 1;
      /*
      **   If this section is reverse-aligned, or unordered,
      **   do not allocate any padding here.
      */
      if (PIC30_IS_REVERSE_ATTR(now_seg) || PIC30_IS_UNORDERED_ATTR(now_seg))
        alignment = 2;
#endif
...
#if defined(TC_PIC30) && defined(tc_frob_symbol)
          /* call target-specific function before any symbols are removed */
...
#ifdef TC_PIC30
              /* BIN30-61: Remove symbols that are undefined
                 and not used in a relocation, such as .extern
                 symbols added to satisfy .ifdecl, .ifndecl  */
              || ( S_GET_SEGMENT (symp) == undefined_section &&
                   ! symbol_used_in_reloc_p (symp))

              /* Remove register symbols*/
              || (S_GET_SEGMENT (symp) == reg_section)
#endif
...
#ifdef TC_PIC30
      /* The value of a symbol is expressed in PC units;
         must convert to octets to match the frag address.  */
     ...
     segT segment;
{
  ...
  for (fragP = segment_frag_root; fragP; fragP = fragP->fr_next)
	...
	case rs_org:
#ifdef TC_PIC30
          /* This is needed for CODE sections. For non-CODE sections,
             it does not impact functionality except that attempts
             to .org backwards are detected earlier. */
           address = fragP->fr_offset;
#endif
	case rs_space:
