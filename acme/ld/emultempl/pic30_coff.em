...
/*
** Build a list of READONLY sections
**  in order of decreasing size
**
** - this function is called via bfd_map_over_sections()
*/
static void
pic30_build_readonly_section_list (abfd, sect, p)
     bfd *abfd ATTRIBUTE_UNUSED ;
     asection *sect;
     PTR p;
{
  ...
  */
  if (((sect->flags & SEC_READONLY) == 0)
      || (sect->lma < info->base) || (sect->lma > info->limit))
    return;
...
** size, alignment power, and READONLY status.
**
** If a suitable block is found, fill in the
** offset field to reflect the section starting
** address and return the block.
...
*/
static struct pic30_memory *
pic30_best_fit_memory (lst, size, align_power, readonly)
     struct pic30_memory *lst;
     bfd_vma size;
     unsigned int align_power;
     bfd_boolean readonly;
{
...
*/
static arelent **
bfd_pic30_load_relocs (abfd, sec)
     bfd *abfd;
     asection *sec;
{
  long relsize;
...
bfd_pic30_dump_reloc_table(bfd *abfd, asection *sec)
{
  ...
  rels = bfd_pic30_load_relocs(abfd, sec);
     ...
     bfd *parent;
{
  ...
  if (!bfd_make_readable (abfd))
      {
        fprintf( stderr, "Link Error: can't make jump table readable\n");
        abort();
      }
     ...
     bfd *parent ATTRIBUTE_UNUSED;
{
  ...
  /* create a bare-bones section */
  flags = SEC_HAS_CONTENTS | SEC_IN_MEMORY | SEC_CODE;
  align = 1;
     ...
     bfd *parent;
{
  ...
  /* fill it with RESET opcode */
  d = isr_data;
  *d++ = 0; *d++ = 0;
  *d++ = 0xFE; *d++ = 0;

  /* create global label at offset zero */
  bfd_pic30_create_symbol (abfd, "__DefaultInterrupt", sec, BSF_GLOBAL, 0);
...
#if 0
  printf("pic30_scan_data_section: %s %lx\n", sect->name,
         (long unsigned) sect->flags);
#endif
     ...
     PTR p ATTRIBUTE_UNUSED ;
{
  ...
  return;
} /*static void bfd_pic30_skip_data_sections (...)*/
...
{
  asection *sec;
  lang_input_statement_type *prev = (lang_input_statement_type *) 0;
    ...
    for (p = rels; relcount && *p != (arelent *) NULL; p++, relcount--)
      {
        ...
        if ((q->howto->type == R_PIC30_WORD_HANDLE) ||
            ...
            (q->howto->type == R_PIC30_HANDLE))
          {
            ...
            if (((*(q->sym_ptr_ptr))->flags & BSF_LOCAL) == BSF_LOCAL) 
              {
                char * global_name = (char *) bfd_alloc ( output_bfd,
            ...
            bfd_pic30_add_handle_sym (sym_name);
            handle_relocs++;
  ...
  asection *sec;
{
  ...
  if (pic30_allocate) {  /* if best-fit allocator enabled */
    ...
    /* set implied attributes, in case this
       object file pre-dates v1.30 */
    err = pic30_set_implied_attributes(sec, 0);
    if (err < 0)
      einfo(_("%P%F: Error: Attributes for section %s from object %s%s%s"
              " conflict with implied attributes. (Rebuild the object if"
              " possible; otherwise map %s in the linker script.)\n"),
            sec->name, sec->owner->filename,
            sec->owner->my_archive ? " in library " : "",
            sec->owner->my_archive ? outer_archive_name(sec->owner) : "",
            sec->name);
