...
#if PIC30
lang_output_section_statement_type *pic30_current_os;
...
#if PIC30
/* FIXME: Should we test PIC30_SECTION_IN_PROGRAM_MEMORY() instead? */
#define PIC30_PROGRAM_MEMORY_ADDR(sec) \
  ((sec->flags & SEC_HAS_CONTENTS) && (!(sec->flags & SEC_DATA)) && \
   (sec->lma == sec->vma))
#endif
...
void lang_select_startup_file(const char* file, int num) {
  const char *s1 = "declared as undefined symbol";
  switch (num)
    {
    case 0:  /* record file associated with __resetPRI */
      ldlang_add_undef("__resetPRI");
      pic30_startup0_file = file;
      if (pic30_debug) {
        printf("\nCRT0 startup module specified as \"%s\"\n", file);
        printf("    __resetPRI %s\n", s1);
      }
      break;

    case 1:  /* record file associated with __resetALT */
      ldlang_add_undef("__resetALT");
      pic30_startup1_file = file;
      if (pic30_debug) {
        printf("\nCRT1 startup module specified as \"%s\"\n", file);
        printf("    __resetALT %s\n", s1);
      }
      break;
  ...
  if (section->output_section == NULL)
	...
	init_os (output);
      ...
      /* Validate the attributes for this input section */
      {
        ...
        else if (!pic30_is_valid_attributes((input_map | output_map), 0))
...
#if PIC30
      /*
      ** Copy pic30-specific attributes to output section
      */
      section->output_section->near = section->near;
      section->output_section->persistent = section->persistent;
      section->output_section->xmemory = section->xmemory;
      section->output_section->ymemory = section->ymemory;
      section->output_section->psv = section->psv;
      section->output_section->eedata = section->eedata;
      ...
      section->output_section->boot = section->boot;
      section->output_section->secure = section->secure;
      ...
      section->output_section->eds = section->eds;
...
#if PIC30
  if ((section != NULL) && (section->_raw_size == 0))
    return;
#endif
...
#if PIC30
          if (PIC30_PROGRAM_MEMORY_ADDR (section))
            minfo ("  0x%V %W", section->vma, section->_raw_size / opb);
          else
            minfo ("    0x%Y %W", section->vma, section->_raw_size / opb);
#else
	  minfo ("0x%V %W", section->vma, section->_raw_size / opb);
#endif
	  ...
	  if (output_section_statement->load_base != NULL)
	    {
              ...
              /* use LMA as load address, since exp_get_abs_int()
                 returns an incorrect value w/ initialized data sections */
...
#ifdef PIC30
    if (!PIC30_PROGRAM_MEMORY_ADDR (result.section->bfd_section))
      minfo ("    0x%Y", value);
    else
      minfo ("  0x%V", value);
#else
...
static void
pic30_init_symbol_list(struct pic30_symbol **lst)
{
...
static void
pic30_add_to_symbol_list(struct pic30_symbol **lst,
                         const char *name,
                         bfd_vma value,
                         int format)
{
  ...
  (*lst)[pic30_symbol_count].format = format;
  ++pic30_symbol_count;
}

static int
pic30_compare_symbol_value (const void *p1, const void *p2)
{
...
static void pic30_print_sorted_symbols (struct pic30_symbol *lst, int count)
{
  int i,j;

  qsort(lst, count,
        sizeof(struct pic30_symbol), pic30_compare_symbol_value);

  for (i=0; i<count; i++) {
    for (j = 0; j < SECTION_NAME_MAP_LENGTH; j++)
      print_space ();
    if (lst[i].format == PIC30_CODE)
      minfo ("  0x%V   ", lst[i].value);
    else
      minfo ("    0x%Y   ", lst[i].value);
    minfo ("             %T\n", lst[i].name);
  }
}
...
#if PIC30
      /* record the symbol */
      bfd_vma value = hash_entry->u.def.value
                      + hash_entry->u.def.section->output_offset
                      + hash_entry->u.def.section->output_section->vma;

      int format = (PIC30_PROGRAM_MEMORY_ADDR
                    (hash_entry->u.def.section->output_section)) ?
                   PIC30_CODE : PIC30_DATA;

      pic30_add_to_symbol_list (&pic30_symbol_list, hash_entry->root.string,
                                value, format);
#else
      /* print the symbol */
      int i;
...
#if PIC30
          if (PIC30_PROGRAM_MEMORY_ADDR (i->output_section))
            minfo ("  0x%V %W %B\n",
...
#if PIC30
          pic30_print_sorted_symbols (pic30_symbol_list, pic30_symbol_count);
...
#if PIC30
  if (PIC30_PROGRAM_MEMORY_ADDR (s->output_section))
    minfo ("  0x%V %W", addr, s->size);
  else
    /* use another format for data memory; ignore phantom bytes */
    minfo ("    0x%Y %W", addr, s->size / opb);
#else
  minfo ("0x%V %W ", addr, s->size);
#endif
  ...
  for (s = output_bfd->sections; s != NULL; s = s->next)
      ...
      for (os = output_bfd->sections; os != s; os = os->next)
	{
	  ...
	  */
	  ...
	  einfo (
_("%X%P: Link Error: section %s [%V -> %V] overlaps section %s [%V -> %V]\n"),
     ...
     bfd_vma base;
{
  if ((region->current < region->origin
	...
	{
	  einfo (_("%X%P: Link Error: address 0x%v of %B section %s is not within region %s\n"),
		 region->current,
	...
	{
	  einfo (_("%X%P: Link Error: region %s is full (%B section %s)\n"),
		 region->name,
		 os->bfd_section->owner,
		 os->bfd_section->name);
	}
     ...
     bfd_boolean check_regions;
{
  ...
  for (; s != (lang_statement_union_type *) NULL; s = s->header.next)
	...
	case lang_output_section_statement_enum:
		...
		if (os->addr_tree == (etree_type *) NULL)
		...
		else
		  {
...
#if PIC30
                    /* Make os visible in a global variable.
                       Then exp_fold_tree() can check if a region
                       has been specified. If ALIGN() and NEXT()
                       appear in the expression, they should
                       reference the specified region, not
                       the previous section. SSR# 21589 */
                    pic30_current_os = os;
#endif
		    r = exp_fold_tree (os->addr_tree,
				       ...
				       dot, &dot);
#if PIC30
                    /* clear the above-mentioned global variable */
                    pic30_current_os = 0;
#endif
		    if (!r.valid_p)
...
#if PIC30
                /* If a load address was specified, apply it
                   to the output section now. Updates to the
                   region must use the lma, not the vma. */
		if (os->load_base != (etree_type *) NULL)
                  {
		    etree_value_type r;

                    r = exp_fold_tree (os->load_base,
                                       abs_output_section,
                                       lang_allocating_phase_enum,
                                       dot, &dot);
      ...
      **
      ** Compare this to the list of unassigned sections,
      ** which may include absolute sections. Call
      ** pic30_set_output_section_flags() to update the flags
      ** for any sections created in the linker script.
      **
      ** If we happen to land on top of an absolute
      ** section, skip past it while respecting alignment.
      **
      ** This logic is similar to lang_check_section_addresses(),
      ** but we can't factor out the common parts without hacking
      ** up the base GNU code more than we want to.
      */
      if (pic30_avoid_absolute_sections) {
        extern void pic30_set_output_section_flags
          (lang_output_section_statement_type *);
        extern struct pic30_section *unassigned_sections;

        extern bfd_boolean pic30_debug;
        unsigned int opb = bfd_octets_per_byte (output_bfd);
        struct pic30_section *s,*next;

        bfd_vma os_start = bfd_section_lma (output_bfd, os->bfd_section);
        bfd_vma os_len   = bfd_section_size (output_bfd,
                                            os->bfd_section) / opb;
        bfd_vma os_end   = os_start + os_len - 1;
        bfd_vma s_start, s_end;
        

#define SEC_IN_PROG_SPACE(s) \
  (PIC30_IS_CODE_ATTR(s) | PIC30_IS_PSV_ATTR(s) | PIC30_IS_EEDATA_ATTR(s) \
   | PIC30_IS_AUXFLASH_ATTR(s))
        ...
        if (os->bfd_section->_raw_size > 0) {
          for (s = unassigned_sections; s != NULL; s = next) {
            ...
            if (s->sec &&
                ...
                !PIC30_IS_EXTERNAL_ATTR(s->sec)) {
              s_start = s->sec->lma;
              s_end   = s_start + (s->sec->_raw_size / opb) - 1;
              if ((SEC_IN_PROG_SPACE(os->bfd_section) &&
                   SEC_IN_PROG_SPACE(s->sec)) ||
                  (SEC_IN_DATA_SPACE(os->bfd_section) &&
                   SEC_IN_DATA_SPACE(s->sec))) {
                if (pic30_debug)
                  printf("  checking for overlap between %s and %s\n",
...
#if PIC30
	    dot = os->bfd_section->lma + os->bfd_section->_raw_size / opb;
#else
...
/* Check that the architectures of all the input files are compatible
...
lang_check ()
{
  ...
  for (file = file_chain.head;
	  ...
	  && (bfd_get_file_flags (input_bfd) & HAS_RELOC) != 0)
	{
      ...
      if (compatible == NULL && pic30_force_link == 0)
	{
...
lang_reset_memory_regions ()
{
  ...
  for (o = output_bfd->sections; o != NULL; o = o->next)
    o->_raw_size = 0;
  /* FIXME -- This code is probably no longer needed, and doesn't
     build after the merge of binutils-2_14 -- GM 12/19/2003 */
/* #if PIC30 */
/*   /\* clean up the result of "AT>LMAregion" so lang_size_sections() */
/*      can be called more than once *\/ */
/*   { */
/*     lang_statement_union_type *s = statement_list.head; */
...
lang_process ()
{
  ...
  pic30_avoid_absolute_sections = FALSE;
  if (last_os != (lang_output_section_statement_type **) statement_list.tail)
      lang_size_sections ((lang_statement_union_type*) (*last_os),
			  abs_output_section,
			  (lang_statement_union_type**) last_os, 0, (bfd_vma) 0, 
			  NULL, TRUE);
#endif
