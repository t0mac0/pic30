     ...
     const char *name;
{
...
#if defined(PIC30)
  /* For absolute PSV sections, the vma is derived from,
      but not equal to, the section header address. */
  if ((hdr->sh_flags & SHF_PSV) &&
      (hdr->sh_flags & SHF_ABSOLUTE))
    newsect->vma = PSV_BASE + (hdr->sh_addr & PSV_MASK);
#endif
  ...
  if (hdr->sh_flags & SHF_NEAR)
    PIC30_SET_NEAR_ATTR(newsect);
  if (hdr->sh_flags & SHF_PERSIST)
    PIC30_SET_PERSIST_ATTR(newsect);
  if (hdr->sh_flags & SHF_XMEM)
    PIC30_SET_XMEMORY_ATTR(newsect);
  if (hdr->sh_flags & SHF_YMEM)
    PIC30_SET_YMEMORY_ATTR(newsect);
  if (hdr->sh_flags & SHF_PSV)
    PIC30_SET_PSV_ATTR(newsect);
  if (hdr->sh_flags & SHF_EEDATA)
    PIC30_SET_EEDATA_ATTR(newsect);
  if (hdr->sh_flags & SHF_ABSOLUTE)
    PIC30_SET_ABSOLUTE_ATTR(newsect);
  if (hdr->sh_flags & SHF_REVERSE)
    PIC30_SET_REVERSE_ATTR(newsect);
  if (hdr->sh_flags & SHF_DMA)
    PIC30_SET_DMA_ATTR(newsect);
  if (hdr->sh_flags & SHF_BOOT)
    PIC30_SET_BOOT_ATTR(newsect);
  if (hdr->sh_flags & SHF_SECURE)
    PIC30_SET_SECURE_ATTR(newsect);
  if (hdr->sh_flags & SHF_MEMORY) {
    PIC30_SET_MEMORY_ATTR(newsect);
...
#if defined(PIC30)
	 	    /*
		    ** I think this will work, with the additional
		    ** address check above.
		    ** The calculation below did not work.
		    */
		    newsect->lma = (phdr->p_paddr
				    + hdr->sh_addr - phdr->p_vaddr);
#else
...
#if PIC30_DEBUG
  printf("_bfd_elf_make_section_from_shdr: %s, map = %x\n",
         name, pic30_attribute_map(newsect));
  printf("_bfd_elf_make_section_from_shdr: %s, vma = %lx, lma = %lx, len = %lx\n",
         name, newsect->vma, newsect->lma, newsect->_raw_size/2);
#endif
     ...
     PTR failedptrarg;
{
...
#if PIC30_DEBUG
  printf("\nelf_fake_sections: %s, vma = %lx, lma = %lx, _raw_size = %lx\n",
         asect->name, asect->vma, asect->lma, asect->_raw_size);
...
#if defined(PIC30)
               )) /* allow noload sections to keep their contents */
#else
...
#if defined(PIC30)
  if (PIC30_SECTION_IN_DATA_MEMORY(asect))
    this_hdr->sh_flags |= SHF_WRITE;
#else
  if ((asect->flags & SEC_READONLY) == 0)
    this_hdr->sh_flags |= SHF_WRITE;
#endif
    if (((asect->flags & SEC_CODE) != 0) || (asect->auxflash != 0))
  ...
  /*
   *
   * Convert any pic30-specific attributes.
   *
   */

#if PIC30_DEBUG
  printf("\nelf_fake_sections: %s, initial sh_flags = %lx\n",
         asect->name, this_hdr->sh_flags);
  printf("elf_fake_sections: %s, map = %x\n",
         asect->name, pic30_attribute_map(asect));
#endif

  if (PIC30_IS_INFO_ATTR(asect))
    this_hdr->sh_type = SHT_NOTE;

  if (PIC30_IS_NEAR_ATTR(asect))
    this_hdr->sh_flags |= SHF_NEAR;
  if (PIC30_IS_PERSIST_ATTR(asect))
    this_hdr->sh_flags |= SHF_PERSIST;
  if (PIC30_IS_XMEMORY_ATTR(asect))
    this_hdr->sh_flags |= SHF_XMEM;
  if (PIC30_IS_YMEMORY_ATTR(asect))
    this_hdr->sh_flags |= SHF_YMEM;
  if (PIC30_IS_PSV_ATTR(asect))
    this_hdr->sh_flags |= SHF_PSV;
  if (PIC30_IS_EEDATA_ATTR(asect))
    this_hdr->sh_flags |= SHF_EEDATA;
  if (PIC30_IS_ABSOLUTE_ATTR(asect))
    this_hdr->sh_flags |= SHF_ABSOLUTE;
  if (PIC30_IS_REVERSE_ATTR(asect))
    this_hdr->sh_flags |= SHF_REVERSE;
  if (PIC30_IS_DMA_ATTR(asect))
    this_hdr->sh_flags |= SHF_DMA;
  if (PIC30_IS_BOOT_ATTR(asect))
    this_hdr->sh_flags |= SHF_BOOT;
  if (PIC30_IS_SECURE_ATTR(asect))
    this_hdr->sh_flags |= SHF_SECURE;
  if (PIC30_IS_MEMORY_ATTR(asect))
  ...
  printf("elf_fake_sections: %s, sh_flags = %lx\n",
         asect->name, this_hdr->sh_flags);
  printf("elf_fake_sections: %s, sh_addr = %lx, sh_size = %lx, sh_info = %lx\n",
         asect->name, this_hdr->sh_addr, this_hdr->sh_size, this_hdr->sh_info);
#endif
...
Error: First section in segment (%s) starts at 0x%x whereas the segment starts at 0x%x"),
	  ...
	  if (! m->p_flags_valid)
		...
		p->p_flags |= PF_X;
#if defined(PIC30)
              if (PIC30_SECTION_IN_DATA_MEMORY(sec))
#else
