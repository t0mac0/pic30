...
#ifdef tc_frob_label_before_define
  tc_frob_label_before_define (sym_name);
#endif
  ...
  if (symp->sy_resolving)
	...
	do_symbol:
	  ...
	  /* If we have equated this symbol to an undefined or common
	     ...
	     is equated.  */
          /* but if we have equated it to an equate of an undefined symbol,
             then we want to inrease final_val by the sum of the offsets in
             the chain of equates, don't we? */
	  if (! S_IS_DEFINED (add_symbol) || S_IS_COMMON (add_symbol))
	      ...
	      if (finalize_syms)
		{
		  ...
		  symp->sy_value.X_add_number = final_val + left;
...
#if 1
  /* if we have a symbol that refers to another symbol, should we mark the
     other symbol as being used inside a relocation too?   This would be
     eggspecially useful if the symbol we first thought of is an .equ or the
     like */
  if (symbol_equated_reloc_p(s))
    symbol_mark_used_in_reloc(s->sy_value.X_op_symbol);
#endif
}
