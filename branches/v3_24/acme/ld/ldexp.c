      ...
      switch (tree->type.node_code)
	{
	    ...
	    case NEXT:
	      if (allocation_done != lang_first_phase_enum)
                {
                  /* If region has been specified, align to region->current.
                     See lang_size_sections() in ldlang.c for more info.  */
                  if ((pic30_current_os) &&
                      strcmp (pic30_current_os->region->name, "*default*") != 0)
                    {
                      result.value = align_n (pic30_current_os->region->current,
                                              result.value);
                    }
                  else
...
#ifdef PIC30
          /* BIN30-157: return a valid output section statement here */
	  result.section = abs_output_section;
#else
	  result.section = 0;
#endif
